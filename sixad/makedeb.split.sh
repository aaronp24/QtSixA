...
ARCH="powerpc"; make_dirs; copy_files; make_arch; copy_32
ARCH="ppc64"; make_dirs; copy_files; make_arch; copy_64
  ...
  if [ -f ./sixad_deb_i386.deb ]; then
    {
      ...
      {
	if [ -f ./sixad_deb_ppc64.deb ]; then
	{
	  rm -r ./sixad_deb_amd64
	  rm -r ./sixad_deb_i386
	  rm -r ./sixad_deb_powerpc
	  rm -r ./sixad_deb_ppc64
	  ...
	  mv sixad_deb_ppc64.deb Debs/sixad_"$VERSION"_ppc64.deb
	  echo "Done!"
	}
	else
	{
	  ...
	  rm -r ./sixad_deb_powerpc
	  rm -r ./sixad_deb_ppc64
	  echo "Error building the ppc64 package"
	}
	fi
	}
      else
